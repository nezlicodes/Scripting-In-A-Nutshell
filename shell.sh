...
printf "\t - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n"
printf "\n"
if [[ ! -f "$DNAFILE" || ! -f "$DNA2FILE" || ! -f "$DOTSECRET" || ! -d "$EMPTY_FOLDER" || -d "$DNA_SEQUENCES" ]]; then
    show_error
    exit 1
fi
...
read -p "" dir
while [[ $dir != "mkdir dna_sequences" ]]; do
...
read -p "" dir
while [[ $dir != "cp dna.txt dna_sequences" ]]; do
...
read -p "" dir
while [[ $dir != 'rm -r empty_folder' ]]; do
    printf " Type: ${RED}rm -r empty_folder${RESET}\n\t$ID$ "
    read -p "" dir
done
...
read -p "" dir
while [[ $dir != "mv dna2.txt dna_sequences" ]]; do
    printf " Type: ${RED}mv dna2.txt dna_sequences${RESET}\n\t$ID$ "
    read -p "" dir
done
